...
function CreatePlayer({ panel }) {
    ...
    return (
        <>
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-2/3 flex flex-col gap-8 w-3/4 md:w-1/3 h-auto bg-stone-300 dark:bg-stone-700 p-4 rounded-md ">
                ...
                <form className="flex flex-col flex-wrap justify-around gap-4" onSubmit={(e) => { submitPlayer(e) }}>
                    ...
                    <div className="flex flex-col justify-center gap-1">
                        ...
                        <Listbox value={type} onChange={setType} >
                            <ListboxButton className="flex items-center gap-2 border px-4 py-2 border-2 outline-slate-100 border-slate-100 text-slate-900 dark:text-slate-100 dark:text-slate-100 rounded-sm capitalize" id="type" name="type">
                                {getSVGByPlayerType(type, 'text-purple-400')}
                                <span className="text-fuchsia-400">{type}</span>
                            </ListboxButton>

                            <ListboxOptions className="px-2 py-1 border borde-blue-500 text-slate-900 dark:text-slate-100 hover:outlie dark:text-slate-100 rounded-sm capitalize">
                                {players.map((player) => (
                                    <ListboxOption
                                        key={player}
                                        value={player}
                                        className="flex items-center gap-2 px-4 py-2 hover:bg-pink-200 cursor-pointer"
                                    >
                    ...
                    <button type="submit" className="p-2 text-slate-800 bg-violet-500 rounded-md font-bold font-poppins">Submit</button>
