...
function UpdateCharging({ charging, panel }) {
    ...
    const [player, setPlayer] = useState(charging?.player || null);
    const [players, setPlayers] = useState([]);

    const [chargingStartDate, setChargingStartDate] = useState(charging?.chargingStartDate);
    const [chargingEndDate, setChargingEndDate] = useState(charging?.chargingEndDate);

    const [chargingStartTime, setChargingStartTime] = useState(charging?.chargingStartTime);
    const [chargingEndTime, setChargingEndTime] = useState(charging?.chargingEndTime);
    ...
    const submitCharging = async (e) => {
        ...
        try {
            ...
            let res = await fetch(`${BACKEND_URL}/charging/${charging._id}`, {
                method: "PUT",
                headers: {
                    "content-type": "application/json"
                },
    ...
    return (
        <>
            <div className="fixed top-1/2 left-1/2 -translate-1/2  flex flex-col gap-8 w-3/4 md:w-1/3 max-h-9/10 overflow-y-scroll scrollbar-thin  h-auto bg-stone-300 dark:bg-stone-700 p-4 rounded-md z-100">

                <span className="text-rose-400 font-bold font-poppins text-lg">Update Charging</span>
